...
func (r *RepositoryMongoProjects) UpdateProjectName(service string, project *entitiesDB.MdProject) error {
	collection := r.db.Database(service).Collection("projects")
	...
	memberFilter := bson.M{
		"members.userId": project.Members[0].UserID,
		"members.role":   "O",
	}
...
func (r *RepositoryMongoProjects) DelProjectName(service string, project *entitiesDB.MdProject) error {
	collection := r.db.Database(service).Collection("projects")
...
func (r *RepositoryMongoProjects) UpdateMemberRole(
	...
	filter := bson.M{
		...
		"members": bson.M{
			"$elemMatch": bson.M{
				"userId": ownerID,
				"role":   "O", // Owner должен существовать
			},
		},
...
func (r *RepositoryMongoProjects) GetProjectsByUserOwnerID(service string, userID primitive.ObjectID) ([]entitiesDB.MdProject, error) {
	collection := r.db.Database(service).Collection("projects")
...
func (r *RepositoryMongoProjects) GetProjectByUserOwnerID(service string, project *entitiesDB.MdProject, userId primitive.ObjectID) error {
	collection := r.db.Database(service).Collection("projects")
