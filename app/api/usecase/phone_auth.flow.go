...
func (s *ServiceAuth) ValidSmsUserFlow(ctx *gin.Context, input *DTO.SmsValid) (DTO.AnswerRegToken, error) {
	...
	//create TOKEN----------------------------------------------------
	refreshToken, err := utils.CreateRefreshToken(userSession.ID.Hex(), s.config.RefreshTokenSecret, 19)
	if err != nil {
...
func (s *ServiceAuth) PhoneLoginFlow(ctx *gin.Context, input *DTO.LoginUserValid) (DTO.AnswerRegToken, error) {
	...
	UserID, hashedPassword, err := s.ath.GetPhoneVerificationUserID(ctx.GetString(middlewares.Service), number, true)
	if err != nil {
		if errors.Is(err, mongo.ErrNoDocuments) {
	...
	projectMap, err := s.pr.GetProjectsListByUser(ctx.GetString(middlewares.Service), ctx.GetString(middlewares.Domain), UserID)
	if err != nil {
		return answer, &schemes.ErrorResponse{Code: 105, Err: "Couldn't create a project", ErrBase: err}
	}
